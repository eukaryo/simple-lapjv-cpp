...
private:
...

	{
		...
		free_ = new row[dim];       // list of unassigned rows.
...
public:
	...
	static int my_lapjv(const vector<vector<int>>&costmat) {
		assert(1 <= costmat.size());
		assert(1 <= costmat[0].size());
		const int H = costmat.size(), W = costmat[0].size();
		REP(i, 1, H)assert(costmat[i].size() == W);

		const int N = max(H, W);
		vector<vector<cost>>assigncost(N, vector<cost>(N, 0));
		rep(i, H)rep(j, W)assigncost[i][j] = costmat[i][j];
		...
		cost c = lap(N, assigncost, rowsol, colsol, u, v);
